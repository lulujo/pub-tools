...
- **Rank Math SEO (PUB-6, Done):** SEO meta fields (`rank_math_description`, `rank_math_focus_keyword`, etc.) are REST-writable. The enabling PHP snippet lives in the **Kadence child theme** (`wp-content/themes/kadence-child/functions.php`) &mdash; Blackbird's active theme is **Kadence**; the child theme holds custom PHP so it survives Kadence updates. Posting skills set SEO automatically; backfill old posts with `integrations/wordpress/seo_backfill.py`. See `integrations/wordpress/RANK_MATH_SEO.md`.
